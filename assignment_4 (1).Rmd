...
```{r}
summary_0 <- datasummary(data = GrowthSW, growth + rgdp60 ~ mean + median+ sd+min+max)
model1 <- lm(growth~ treat, data=GrowthSW)
model2<- lm( model1, ~, control_var1)
model3<-lm( model2, ~, control_var2)
model4<-lm( model3, ~, control_var3)
```
