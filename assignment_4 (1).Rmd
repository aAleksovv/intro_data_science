...
author: |
  | Andrian Aleksov
  | a.aleksov@students.uu.nl
  | http://www.github.com/aAleksovv
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
new_data <- GrowthSW|>mutate(treat=if_else(revolutions>0, 1,0))
new_data |> datasummary( formula = growth +rgdp60~(mean+median+sd+min+max) )


```

**In this table is possible to see  the mean, median, standard deviation, minimum, and maximum of growth and rgdp60. There is also the differentiation of countries that had revolutions and those that didn't, through the variable 'treat'. Andrian used the function 'datasummary' to create the final tidy table with the result**: type one or two sentences describing this table of a fellow student below. For example, comment on the mean and median growth of both groups. Then stage, commit and push it to their github repository. 
...
The p-value of the test = 0.06871
The p-value test is the probability that a statistical measure, in this case the mean of the t distribution is greater or equal to the observed results. It represents the minimum required value to reject the null hypothesis. It is a statistical measure to determine the strength of the evidence against a null hypothesis. In other words, the P VALE quantifies the probability of obtaining results as extreme as, or more extreme than, the observed data under the assumption that the null hypothesis is true. In this case there is a strong evidence (p-vlaue=0.06871) against the null hypothesis, and we may reject in favor of an alternative hypothesis. 
...
"rgdp60" is used as an anchor or a reference as to look at the data in terms of 1960 value/worth.

We now want to estimate a stepwise model. Stepwise means that we first estimate a univariate regression 
$\text{growth}_i = \beta_0 + \beta_1 \cdot \text{treat}_i + \epsilon_i$ , and in each subsequent model, we add one control variable. 
...
#ANSWER HERE# 
The statistical analysis shows that education is the main driver of economic growth. This can be related to previous assignments where we analyzed the influence of education and gdp. We can conclude that education, due to a number of factors is the main driver of economic growth. 
...
```{r}
library(kableExtra)
table_1 <- list(model1, model2, model3, model4) |>
  modelsummary(stars=T, gof_map = c("nobs", "r.squared")) |>
